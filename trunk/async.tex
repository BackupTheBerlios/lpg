...
{\it delta} pikseli ruch stanie siê szybszy zale¿nie od tej warto¶ci. 
\item {\it int baud} \\
prêdko¶æ w bitach na sekundê z jak± pracuje myszka ( normalnie 1200 ).
\item {\it int delta}\\
ilo¶æ pikseli o ile nale¿y przesun±æ myszkê zanim zostanie
w³±czona akceleracja .
\item {\it char *device}\\
...
mojej myszki ;).
\item {\it int slack}\\
je¿eli warto¶æ wynosi -1 próba przesuniêcia myszy poza granicê obrazu
pozostawia kursor na miejscu. Je¿eli $>=$ 0 kursor pojawi siê z drugiej
strony po przesuniêciu go o {\it slack} pikseli poza granicê ekranu.
\item {\it int maxx}\\
rozdzielczo¶æ pozioma terminala. Przy domy¶lnej czcionce znak ma 10 pikseli szeroko¶ci
co daje nam rozdzielczo¶æ 10*80-1.
\item {\it int maxy}\\
rozdzielczo¶æ pionowa terminala. Przy domy¶lnej czcionce znak ma 12 pikseli wysoko¶ci
