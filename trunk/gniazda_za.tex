...
przyszedl czas na poszerzenie wiedzy na ten temat. Zaznajomimy siê z kilkoma
...
Funkcje te to {\bf setsockopt()} i {\bf getsockopt()}. Zaczniemy wla¶nie od nich:
\begin{verbatim}
#include <sys/types.h>
...
\item {\tt level}   - Poziom, na którym bêdziemy dokonywaæ operacji. Omawiane funkcje
          udostêpniaj± dostêp do manipulownia opcjami od poziomu gniazda
...
\end{verbatim}
                    ...
                    multicastów tylko na gniazdach SOCK\_DGRAM i SOCK\_RAW.

                    Znaczenie poszczególnych pól struktury ip\_mreqn powinno
                    byæ jasne. Tylko jedna uwaga: nie nalezy myliæ opisywanych
                    obecnie adresów multicast w warstwie IP z adresami
                    multicast warstwy wizycznej. Tymi ostatnimi zajmiemy sie
                    pó¼niej.
...
\item {\tt TCP\_NODELAY} - Je¶li równe 1 to zostaje wy³±czony tzw. algorytm Nagle. Mowi±c
              ...
              nie zostanie uzbierana wystarczaj±ca paczka danych. Szczególy w
...
\item {\tt cmd} - Komenda. W operacjach na gniazdach przydatne sa:
\begin{itemize}
...
\item {\tt F\_SETOWN} - Wskazuje proces (PID) albo grupê procesów, które maj±
                 ...
                 procesów, dla odróznienia od pojedynczych procesów, podaje siê
...
\subsection{ioctl()}
O funkcji {\bf ioctl()} mo¿naby mówiæ d³ugo. W za³o¿eniu przeznaczona do 
...
\end{itemize}
Wszystkich ioctl'sow jest bardzo du¿o. Praktycznie ka¿de urz±dzenie udostêpnia
jakis ioctl. Ca³kiem obfit± listê mo¿na zobaczyæ wydaj±c polecenie {\sl man
ioctl\_list}. Natomiast wszystkie ioctls ukierunkowane na sterowanie szeroko
...
\item {\tt FIOCSETOWN}, {\tt FIOCGETOWN} - dokladnie to samo, co wywo³anie {\bf fcntl()} z argumentami
                         F\_SETOWN/F\_GETOWN ...
...
j±dra. Mowi±c wprost jest ona programowym interfejsem dostêpu do plików
...
Wa¿ne jest aby pamietaæ, ¿e SIOCGIFCONF wype³nia tylko pole ifr\_name - reszta
pól ma zupe³nie przypadkow± warto¶æ ! Do uzyskania reszty informacji o danym
interfejsie slu¿± oddzielne polecenia {\bf ioctl()} ({\sl linux/sockios.h}):
\begin{itemize}
...
\begin{verbatim}
   ...
   /* Ustawiamy dwie flagi mówi±ce, ¿e dana scie¿ka jest aktywna  */
...
\end{verbatim}
          Pola events oraz revents mog± przyjmowaæ m.in. takie warto¶æi:
\begin{itemize}          
...
\end{itemize}
          Reszta warto¶æi znajduje siê w {\sl bits/poll.h}
...
\begin{verbatim}
 ...
 while (1) {
   ...
   /* gniazd nie zmieni swego stanu, co bêdzie oznaczalo, ¿e          */
...
wstrzymywany na bli¿ej nieokre¶lony czas ({\bf select()}). Oczekuje on bezczynnnie
...
doaj±c flagê {\tt MSG\_OOB} do wywo³ania {\bf recv()}:
\begin{verbatim}
...
\end{verbatim}
Po kilku linijkach widzimy, ¿e niezupe³nie o ten plik nam chodzilo. Co teraz ?
Czy musimy d³ugo czekaæ a¿ ten ogromny plik zostanie wy¶wietlony ? Otó¿ nie. 
Zazwyczaj wciskamy ctrl+c i po chwili mamy znowu dostêp do shella. Spróbujmy
dla odmiany zamiast ctrl+c wpisaæ dowolne polecenie shella. Zauwa¿ymy, ¿e nie
...
skorzystaæ z uslug inetd'a. Ten super-serwer zbli¿a kwestiê programowania
demonów sieciowych do poziomu zwyklych programików korzystaj±ych ze
standardwowych strumieni I/O.

Jak dziala inetd ? Przy starcie odczytuje swój plik konfiguracyjny (zazwyczaj
...
Kiedy wreszcie wykryje, ze ktos "puka do drzwi" wywo³uje {\bf accept()} i tworzy
...
X i Y s± takie same, jak w odpowiadj±cej jej czê¶ci master. Linux wspiera
...
\subsection{sendfile()}
Ta wygodna funkcja zostala wprowadzona dopiero w linuxach serii 2.2.x. Slu¿y do
kopiowania danych pomiêdzy deskryptorami (zarówno plików jak i gniazd), a 
prototyp wyglada tak ({\bf sendfile(2)}):
\begin{verbatim}
#include <unistd.h>
...
\end{verbatim}
Pierwsze trzy argumenty maj± znaczenie znalogiczne, jak w wywo³aniu {\bf socket()}.
Ostatni argument to dwie zmienne typu int, którym zostan± przypisane
nowoutworzone deskryptory. Funkcja zwraca 0 albo -1 (b³±d).
...
adresu IP, jak i nazwy domenowej funkcja zwraca stukturê:
\begin{verbatim}
 struct hostent {
...
\subsection{inet\_*()}
Grupa funkcji przeznazcona do konwersji miêdzy ró¿nymi formatami zapisu adresów
internetowych.
\begin{verbatim}
#include <sys/socket.h>
