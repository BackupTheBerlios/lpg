...
kana³u komunikacyjnego. Gniazd u¿ywa siê g³ównie do komunikacji z odleglym
...
Ta uniwersalno¶æ zastosowañ jest zapewniona dziêki istieniu ró¿nych odmian 
gniazd. Gniazdo jest opisywane za pomoc± kombinacji trzech atrybutów: domeny
adresowej, sposobu komunikacji i protoko³u sieciowego. Szczególy poznamy ju¿
za chwilê. 
...
samego - utworzenia naszej (klienta lub serwera) koñcówki pol±czenia (czyli
...
Zagl±dajac tam spostrze¿emy, ¿e zamiennie u¿ywa siê notacji PF\_xxx
...
\item {\tt type}   - Sposob komunikacji. Do wyboru m.in.:
SOCK\_STREAM - dwukierunkowa komunikacja zorientowana na pol±czenia
SOCK\_DGRAM  - komunikacja bezpol±czeniowa korzystaj±ca z tzw. 
                       datagramów
...
PF\_INET u¿ywany jest protoko³ TCP. Natomiast komunikacja 
...
chcemy wysy³aæ pojedyñcze paczki danych i zale¿y nam na szybko¶ci, a
...
W tym momencie stworzyli¶my w³asne gniazdo i tutaj w³a¶nie scie¿ki siê
...
pe³ni±cych fukcjê serwera zajmiemy siê pó¼niej.
...
narazie zajmiemy siê tylko pierwszym przypadkiem.
...
kiedy po³±czenie doszlo do skutku. Je¶li otrzymali¶my warto¶æ 0 to od tej pory
...
{\tt flags} - W tym w³a¶nie parametrze uwidacznia siê przewaga funkcji {\bf recv()/send()}
        ...
        MSG\_OOB     - Onacza chêæ odebrania tzw. danych out-of-band. Trochê
        ...
        MSG\_PEEK    - Powoduje odebranie danych z pocz±tku kolejki danych 
                      ...
                      danych z kolejki. To co¶ w stylu preview ;). Nastêpne
        ...
        MSG\_WAITALL - Je¶li ta flaga nie jest w³±czona to {\bf recv()} odczytuje tyle
                      ...
                      bajtami i je¶li chcemy mieæ pewno¶æ, ¿e otrzymay ca³e
...
korzystaj±cej z protoko³u "tcp". Fukcja ta opiera swoje dzia³anie na zawarto¶ci
...
wyniki od razu przekonwertowane do odpowiedniego formatu (network order), a
...
\end{verbatim}
Jako, ¿e saddr.sin\_port jest warto¶ci± typu short (dwubajtow±) musieli¶my
...
mo¿naby pomin±æ ten krok i wszystko powinno dzia³aæ ale prawdopodobnie nasz
...
nowoutworzonego gniazda lub -1 (b³±d). Wspomnianego gniazda mo¿emy od tej
...
\end{verbatim}
Czêsto pope³nianym b³edem jest tylko jednokrotne wywo³anie {\bf waitpid()} wewn±trz
funkcji. Trzeba pamiêtaæ, ¿e sygna³y w systemach unixowych nie s± ustawiane w 
...
na strumieniach we/wy ({\bf printf()}, {\bf scanf()}, {\bf getchar()} itp.). Zakomentowana jest
...
Wcze¶niejsze rzekierowanie standardowych strumieni nie by³o podyktowane tylko
...
int main (void)
{
  ...
  struct sockaddr_in saddr, /* adres lokalnego gniazda    */
                     caddr; /* adres zdalnego gniazda     */
  char           buf[1024], /* bufor                      */
                  hash[20]; /* bufor na zahashowane has³o */
...
([a-zA-Z0-9./]) i jest stosowane przez algorytm liczenia funkcji hashuj±cej.
Ta sama funkcja ({\bf crypt()}) jest u¿ywana w wiêkszo¶ci systemów unixowych do
...
serwerem. Wybrali¶my procedurê autentyfikacji klienta opart± na dwóch
...
dzia³ania funkcji hashuj±cych na serwerze i na kliencie pokrywaj± siê oraz
...
\end{verbatim}
Blok else wykonuje siê je¶li który¶ z elementów autentyfikacji (has³o, port
¼ród³owy) jest niepoprawny. Tutaj te¿ koñczy siê pêtla while oraz ca³y kod
serwera.

Jakie ró¿nice w stosunku do serwera TCP ? Nie u¿ywalismy funkcji {\bf listen()} oraz
...
Dlaczego ? Poniewa¿ lokalne gniazdo nie by³o skojarzone z pojedyñczym gniazdem
...
Drugim krokiem w tym przypadu jest rêczne przypisanie adresu do lokalnego
...
by zakomentowana oczywi¶cie) u¿ywaæ funkcji {\bf recv()/send()} w miejsce 
{\bf recvfrom()/sendto()}. Po prostu system automatycznie wype³nia³by za nas 
parametry *from i *fromlen tej drugej pary funkcji warto¶ciami podanymi dla
...
przybywaj±cyh datagramów przechowujemy w strukturze wskazywanej przez caddr.
...
podejumuje jedno z trzech dzia³añ:
\begin{itemize}
...
\item ostatnia mo¿liwo¶æ to po prostu nadej¶cie datagramu zawieraj±cego czê¶æ
  przesy³anegopliku; w takim przypadku po prostu wy¶wietlamy ca³± zawarto¶æ
...
uzytkownikami zalogowanymi na tym samym systemie - co¶ w stylu write(1) ale
...
#define CHATDIR       "/tmp/"   /* katalog, w ktorym zak³adane bêd± sockety */
\end{verbatim}
...
który musimy wype³niæ ¶cie¿k± do pliku reprezetuj±cego gniazdo.
...
wiadomo¶æi. Jak widaæ zmieni³ siê sposób obliczania d³ugo¶ci adresu: dodajemy
...
\end{verbatim}
Ustawiamy odpowiednie prawa dostêpu do nowoutworzonego pliku i zarazem do
...
bêdziemy wysy³aæ wiadomo¶ci. W lini komend, jako argument dla programu
...
linijkê wysy³amy do u¿ytkownika, którego podali¶my w linni komend.
\begin{verbatim}
      if (FD_ISSET (sd, &fds))
        {
...
poziomie o szczebel wy¿szym, czyli kiedy oczytujemy dane system usuwa nag³ówek
...
warto¶æi znajduj± siê w pliku {\sl linux/if\_ether.h} .
\begin{verbatim}
...
struct ethhdr
{
        ...
        unsigned short  h_proto;                /* protokól wy¿szej warstwy */
};
\end{verbatim}
...
go na pocz±tek bufora przeznaczonego na ca³± ramkê (pktbuf). Mo¿eby teraz zaj±æ
...
zmieniony paket opakowuje w ramkê ethernetow± i wysy³a pod adres ar\_tha. Kiedy
...
\end{verbatim}
Je¶li {\bf select()} zwróci 0 to znaczy, ¿e up³yn±l limit czasowy.
\begin{verbatim}
    {
...
Na tym na razie poprzestaniemy. Opanowali¶my wszyskie podstawowe umiejêtno¶ci
potrzebne do zaaran¿owania komunikacji przy wykorzystaniu mechanizmu gniazd.
Jak widzieli¶my na przyk³adzie analizowanych programow nasza obecna wiedza na
