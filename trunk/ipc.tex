...
\begin{itemize}
\item Jednokierunkowe potoki z Uniksa
...
\section{Jednokierunkowe potoki z Uniksa}
...
Pod Linuksem, potoki s± obecnie prezentowane wewnêtrznie jako prawid³owe i-wêz³y
...
Na razie potoki s± daleko bezu¿yteczne. Po co zadawaæ sobie trud tworzenia
...
korzystaæ. Przyjmijmy, ¿e potomek przeprowadza jak±¶ akcjê i wysy³a wynik poprzez
...
plików, które u¿ywamy do operacji na potoku. Najczê¶ciej po takiej operacji
...
jest wska¼nikiem do {\tt \&fd[0]}. Po stworzeniu potoku tworzymy naszego potomka.
...
int main(void)
{
        ...
        if(childpid == 0)
        {
                ...
                /* Przesy³amy "string" poprzez koñcówkê wyj¶ciow± potoku */
...
standardowe strumienie. Przyjrzyjmy siê wywo³aniu dup():
...
Pomimo tego, i¿ stary deskryptor mo¿e byæ u¿ywany zamiennie z nowo utworzonym
...
Zamknêli¶my deskryptor pliku 0 ( stdin ), poprzez wywo³anie dup() skopiowali¶my
deskryptor potoku (fd$0$) na standardowe wej¶cie potomka. Po tym wywo³ali¶my
...
jako swoje standardowe wej¶cie! Teraz wszystko co wysy³a rodzic do potoku 
kierowane jest do programu sort.

Istnieje inne wywo³anie systemowe - dup2(), którego mo¿na równie¿ u¿ywaæ.
To wywo³anie oryginalnie powsta³o w 7 Wersji Uniksa i zosta³o przeniesione do
...
Ta standardowa funkcja biblioteki tworzy potok wywo³uj±c wewnêtrznie pipe().
Po tym tworzy proces potomny, uruchamia pow³okê Bourne'a oraz uruchamia
...
Je¿eli ciê podkusi i podasz "rw" pod Linuksem potok zostanie otwarty w trybie
...
dobrodziejstwa masek oraz rozwijania meta-znaków.
...
meta-znaków s± dostêpne! Mo¿emy równie¿ korzystaæ z bardziej zaawansowanych 
...
maksymalny rozmiar bufora dla operacji atomowej na potoku to:
...
bêdzie atomowa. Pod Linuksem limit dla operacji atomowej zosta³ zdefiniowany
w 'linux/limits.h' na:
...
\item Po skoñczeniu wszystkich operacji nazwany potok pozostaje w systemie plików
...
Poniewa¿ FIFO domy¶lnie blokuje program, uruchom go w tle:
...
mamy na my¶li pojedyncz± kolejkê wiadomo¶ci, zestaw semaforów lub segment pamiêci
dzielonej. Identyfikator u¿ywany jest przez j±dro w celu identyfikacji obiektu IPC.
...
zilustrowaæ za³ó¿my, i¿ pos³ugujemy siê warto¶ci± '12345'. Podczas gdy nie
...
\nopagebreak\hrule
\begin{verbatim}
        key_t mojklucz = ftok("/tmp/mojaapp", 'a');
\end{verbatim} 
\nopagebreak\hrule 
\addvspace{\medskipamount}

W powy¿szym przyk³adzie klucz tworzony jest z kombinacji katalogu {\tt /tmp/mojaapp} 
z liter± {\tt 'a'}. Inn± mo¿liwo¶ci± jest u¿ycie bie¿±cego katalogu:
...
Komendê {\tt ipcs} mo¿na u¿yæ w celu otrzymania statusu wszystkich obiektów IPC 
Systemu V. Linuksowa wersja narzêdzia zosta³a stworzona przez {\em Krishna Balasubramanian\/}.
...
ipcs     -m:    Poka¿ tylko pamiêæ dzielon±
...
Powy¿ej widzimy pojedyncz± kolejkê wiadomo¶ci, która ma identyfikator '0'. Jej 
...
struktura typu {\tt msgbuf}. Jest ona zadeklarowana w {\tt linux/msg.h} nastêpuj±co:
...
struct moj_msgbuf {
        ...
        struct  klient_info;    /* Struktura informacji dla klienta */
};
\end{verbatim} 
\nopagebreak\hrule 
\addvspace{\medskipamount}
...
jeden z jej elementów zosta³ usuniêty, a na jego miejsce pojawi³y siê dwa nowe, z
...
\nopagebreak\hrule
\begin{verbatim}
#define MSGMAX  4056   /* <= 4056 */   /* maksymalny rozmiar wiadomo¶ci ( bajty ) */
\end{verbatim} 
\nopagebreak\hrule 
\addvspace{\medskipamount}
...
Jest to wska¼nik do nastêpnej wiadomo¶ci. S± one przechowywane jako
pojedynczo ³±czona lista wewn±trz przestrzeni adresowej j±dra.
...
struct msqid_ds {
    ...
    ushort msg_cbytes;    
    ushort msg_qnum;     
    ushort msg_qbytes;      /* maksymalna ilo¶æ bajtów w kolejce */
...
razem z obiektem zawiera informacje na temat twórcy oraz w³a¶ciciela obiektu ( mog± siê
...
przez maksymaln± liczbê obiektów IPC mog±cych przebywaæ w systemie. Nie przejmuj siê
t± warto¶ci±.
    ...
    ZWRACA: identyfikator kolejki - sukces                                                       
             -1 - b³±d: errno = EACCESS ( brak prawa dostêpu )
					 ...
					 utworzyæ )
                                ...
                                ENOSPC ( maksymalny limit kolejek przekroczony )
  UWAGI: 
\end{verbatim} 
\nopagebreak\hrule 
\addvspace{\medskipamount}
...
kolejka lub wywo³anie zwróci b³±d. U¿ycie tej flagi gwarantuje nam, ¿e nie istnieje
kolejka z otwartym dostêpem.

Mo¿na równie¿ wykonaæ operacjê iloczynu bitowego z flag± podaj±c w ten
sposób tryb dostêpu, gdy¿ ka¿dy
...
Pierwszy argument ( {\tt msgsnd} ) jest identyfikatorem zwróconym przez {\tt msgget}.
...
Je¿eli kolejka jest pe³na wiadomo¶æ nie jest zapisywana, kontrola powraca
...
ipcs}. Teraz zajmijmy siê odbieraniem informacji z kolejki, do tego celu
  ...
  PROTOTYP: int msgrcv ( int msqid, struct msgbuf *msgp, int msgsz, long mtype, int msgflg );
    ZWRACA: ilo¶æ bajtów skopiowan± do bufora wiadomo¶ci
             -1 - b³±d: errno = E2BIG  ( d³ugo¶æ wiadomo¶ci jest wiêksza ni¿ msgsz, 
                                ...
                                EINTR  ( przerwano z powodu nadchodz±cego sygna³u )
...
rozmiar cz³onka {\tt mtype}. Pamiêtaj, ¿e mo¿na to ³atwo obliczyæ:
...
J±dro przeszuka kolejkê w poszukiwaniu najstarszej wiadomo¶ci o podanym typie
...
Przyjrzyjmy siê funkcji odbieraj±cej i kopiuj±cej tê strukturê pod podany adres:
...
Pobieramy kopiê struktury za pomoc± wywo³ania naszej funkcji {\tt get\_queue\_ds}.
...
dla kolejki! Pamiêtaj, ¿e takie obiekty IPC nie znikaj± do czasu poprawnego usuniêcia
...
budowy wewnêtrznej Uniksa na Uniwersytecie Po³udniowej Florydy wpad³em w pewn±
...
tym jak nasza wiadomo¶æ zniknê³a. Aby dokoñczyæ cyklu ¿yciowego kolejki powinni¶my
...
Niewielu mo¿e zaprzeczyæ korzy¶ci p³yn±cych z dostêpu do dok³adnych technicznych wiadomo¶ci.
...
przedstawione w sposób zapewniaj±cy dalsz± naukê i eksperymentowanie.
...
Obecnie u¿ywa on tablic znaków dla danych, umo¿liwiaj±c wysy³anie tekstowych
wiadomo¶ci. Obs³ugê dodatkowych typów danych pozostawiam czytelnikowi jako
æwiczenie.
...
o w³±czeniu IPC Systemu V podczas kompilacji j±dra!

Program {\em utworzy\/} kolejkê wiadomo¶ci je¿eli jeszcze nie istnieje, bez wzglêdu
...
sygna³y, zapewniaj±ce prawid³owo¶æ w ruchu poci±gów. To samo mo¿emy powiedzieæ
...
Ten prosty przyk³ad ukazuje jedynie podstawy, nale¿y sobie zdaæ sprawê, i¿
...
Za³ó¿my, ¿e mamy 5 dostêpnych drukarek. Nasz zarz±dca tworzy zestaw semaforów 
...
zarz±dca zmniejsza o jeden u¿ywany semafor. Teraz warto¶ci± semafora jest 0.
...
Kiedy zadanie Jasia zostanie wykonane zarz±dca zwiêkszy warto¶æ semafora
odpowiadaj±cego za u¿yt± drukarkê. Warto¶ci± semafora staje siê 1 - drukarka jest
dostêpna. Kiedy wszystkie semafory s± ustawione na 1 oznacza to brak dostêpnych drukarek.

Nie daj siê zwie¶æ inicjalizacji semafora jedynk±. Kiedy u¿ywamy semaforów jako
...
Zanim siêgniemy do wywo³añ systemowych przyjrzyjmy siê strukturom wykorzystywanym
podczas operowania semaforami.


\subsubsection{Wewnêtrzne struktury j±dra}

Przyjrzyjmy siê strukturom utrzymywanym przez jadro dla zestawów semaforów.
...
\begin{verbatim}
        ...
        struct semid_ds {
                ...
                struct sem      *sem_base;      /* wka¼nik do pierwszego semafora w tablicy */
...
Czas ostatniej operacji {\tt semop()} ( wiêcej o tym za chwilkê )
...
\begin{verbatim}
        ...
        /* jedna struktura dla ka¿dego semafora w systemie. */
        struct sem {
...
U¿yte z IPC\_CREAT zwraca b³±d je¿eli zestaw ju¿ istnieje.
...
nie u¿ywamy istniej±cego semafora.

Tak jak dla ka¿dej innej formy IPC Systemu V mo¿emy 
wykonaæ operacjê iloczynu bitowego z flag± w celu okre¶lenia praw dostêpu.
...
Maksymalna liczba semaforów jest zdefiniowana w {\tt linux/sem.h\/}:

\begin{verbatim}
        #define SEMMSL  32      /* <=512 maksymalna liczba semaforów dla id */
\end{verbatim}

Zauwa¿, i¿ argument {\tt nsems} jest ignorowany gdy otwieramy ju¿ istniej±cy
zestaw.
...
\begin{verbatim}
  ...
  PROTOTYP: int semop ( int semid, struct sembuf *sops, unsigned nsops);
    ZWRACA: 0 - sukces ( przeprowadzono wszystkie operacje )
            -1 - b³±d: errno = E2BIG ( nsops wiêkszy od maksymalnej liczby 
			       ...
			       ENOMEM ( SEM_UNDO ustawione, brak pamiêci aby utworzyæ strukturê anulowania (undo) )
                               ERANGE ( warto¶æ semafora poza zakresem )
  UWAGI: 
\end{verbatim} 
\nopagebreak\hrule 
\addvspace{\medskipamount}
...
Za³ó¿my, i¿ mamy tylko jedn± drukarkê, która potrafi drukowaæ tylko jedn± pracê.
...
\begin{verbatim}
  ...
  PROTOTYP: int semctl ( int semid, int semnum, int cmd, union semun arg );
    ZWRACA: liczba dodatnia je¿eli sukces 
             -1 - b³±d: errno = EACCESS ( brak prawa dostêpu )
                                EFAULT ( nieprawid³owy adres wskazywany przez argument arg )
                                EIDRM ( zestaw zosta³ usuniêty )
  ...
  UWAGI: Przeprowadza operacje kontrolne na zestawie semaforów
\end{verbatim} 
\nopagebreak\hrule 
\addvspace{\medskipamount}
...
Jest ono analogiczne do wywo³ania {\em msgctl\/}, które jest odpowiednie dla kolejek.
...
pojedyncze jednostki. Do operowania semaforem potrzeba nie tylko klucz IPC, ale
...
Drugi argument ({\tt semun}) to numer semafora, którym jeste¶my zainteresowani.
...
wskazywanej przez {\em array\/} - cz³onka unii.
...
Zwraca warto¶æ pojedynczego semafora z zestawu.
...
Ustawia warto¶æ jednego semafora z zestawu. Warto¶æ pobierana
...
Linuksa, nie znajdziesz ich w innych Uniksach.

Wywo³anie to jest jednym z najtrudniejszych do zrozumienia wywo³añ IPC Systemu V,
dlatego poka¿emy kilka przyk³adów.

Nastêpuj±ca funkcja zwraca warto¶æ podanego semafora. Ostatni argument wywo³ania
...
Za³ó¿my, ¿e potrzebujemy status wszystkich drukarek z naszego przyk³adu:
...
Rozwa¿ nastêpuj±ca funkcjê inicjuj±c± semafor now± warto¶ci±:
...
do niej. Je¿eli ju¿ mówimy o przekazywaniu unii jako argument pozwolê sobie przedstawiæ
...
rzadziej ni¿ z pojedynczej jednostki. Czy potrafisz zlokalizowaæ b³±d w nastêpuj±cym kodzie ?
...
\begin{verbatim}
semtool s ( liczba semaforów w zestawie )
\end{verbatim}

\subparagraph{Blokowanie semafora}

\begin{verbatim}
semtool z ( numer semafora do zablokowania )
\end{verbatim}

\subparagraph{Odblokowywanie semafora}

\begin{verbatim}
semtool o ( numer semafora do odblokowania )
\end{verbatim}
...
void createsem(int *sid, key_t key, int members)
{
        ...
        if(members > SEMMSL) {
                printf("Przykro mi, maksymaln± liczb± semaforów w zestawie jest %d\n",
                        SEMMSL);
                exit(1);
        }
...
void locksem(int sid, int member)
{
        ...
        {
                fprintf(stderr, "Zasoby semafora wyczerpane ( brak blokady )!\n");
                exit(1);
        }
        ...
        if((semop(sid, &sem_lock, 1)) == -1)
        {
                ...
                printf("Zasoby semafora zmiejszone o jeden ( zablokowane )\n");
...
Przyjrzyjmy siê strukturom danych, które j±dro utrzymuje dla pamiêci dzielonej.
...
I tym razem mo¿esz równie¿ wykonaæ operacjê iloczynu bitowego
z flag± w celu okre¶lenia praw dostêpu.
...
Flaga SHM\_RND mo¿e byæ pomno¿ona bitowo z argumentem shmflg aby wymusiæ stronicowe
